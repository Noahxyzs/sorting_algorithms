...
/**
 * new_largest - Find the largest number btween the layers
 ...
 * Return: Nothing.
 */
void new_largest(int *array, size_t size, int i, const int r_size)
{
	int largest = i;
	int lft = (2 * i) + 1;
	int rgt = (2 * i) + 2;
	...
	if (largest != i)
	{
		...
		new_largest(array, size, largest, r_size);
	}
}
...
void heap_sort(int *array, size_t size)
{
	...
	for (i = size / 2 - 1; i >= 0; i--)
		new_largest(array, size, i, r_size);

	for (i = size - 1; i >= 0; i--)
	{
		_swap(array, 0, i, r_size);
		new_largest(array, i, 0, r_size);
	}
}
